    ...
    tabPanel(
      ...
      sidebarPanel(
        ...
        fluidRow(
          ...
          column(12,
                 offset = 0,
                 h5("Input file(s) of model(s)")
          ),
          column(
            12,
            offset = 0,
            downloadButton("download_models", "")
          )
...
server <- shinyServer(function(input, output, session) {
  ...
  observeEvent(input$upload_model,{
    ...
    name_model = input$upload_model$name
    name_model = str_replace(name_model,".txt","")
    
    model = readLines(input$upload_model$datapath)
    
    counter_input$n <- counter_input$n + 1
    
    updateTextAreaInput(session,
                        inputId = paste0("textin_relations_", counter_input$n),
                        value = model
    )
    
    updateTextAreaInput(session,
                        inputId = paste0("textin_relations_name", counter_input$n),
                        value = name_model
    )
  ...
  observeEvent(input$go_v_h, {
    ...
    if (sum(mytable_input[, 2] == "") == 0) {
      ...
      #waitress <- Waitress$new(theme = "line", min = 0, max = 10)
      ...
      for (loop_numb_models in 1:numb_models_to_convert) {
        ...
        if (loop_numb_models < (nrow(mytable_input) + 1)) {
          ...
          feedback_h_models = paste(feedback_h_models,
                                    paste("#", loop_numb_models, " of ", 
                                          numb_models_to_convert, 
                                          ": Creating H-representation of ", 
                                          name_m), sep = "<br>")
          
          #  waiter$update(h1(HTML(feedback_h_models)))
  ...
  output$download_models <- downloadHandler(
    ...
    content = function(file) {
      ...
      n_models = length(names(h_reactive))
      
      for(loop_models in 1:  n_models){
        
        act_model = h_reactive[[names(h_reactive)[loop_models]]]
        n_parameters = ncol(act_model) -2
        all_lines = numeric()
        
        for(loop_ineq in 1 : nrow(act_model)){
          
          sign = ifelse(act_model[loop_ineq,1] == "0", "<", "=")
          right = act_model[loop_ineq,2] 
          left= act_model[loop_ineq,3:ncol(act_model)] 
          
          which_neg = str_detect(left, "-")
          left[which_neg == FALSE] = paste("-",left[which_neg == FALSE],sep="")
          left[which_neg == TRUE] = str_replace(left[which_neg == TRUE] , "-","+")
          
          left_comb = paste(left,paste("p",1:n_parameters,sep=""),sep=" * ")
          
          left_comb = left_comb[left!="-0"]
          
          all =  c(paste(left_comb,collapse=" "),sign,right)
          
          all = paste(all,collapse=" ")
          all = paste(all,";",collapse="")
          
          all_lines = c(all_lines,all)
          
          all_lines = paste(all_lines,collapse= " ")
          
          fileName <-
            paste(names(h_reactive)[loop_models],  ".txt", sep = "")
